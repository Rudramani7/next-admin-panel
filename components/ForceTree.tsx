...
export default function ForceTree() {
  ...
  useEffect(() => {
      ...
      children: [
        {
          name: "Engineering",
          children: [
            { name: "Frontend", children: [{ name: "React" }, { name: "Next.js" }, { name: "TailwindCSS" }] },
            { name: "Backend", children: [{ name: "Node.js" }, { name: "Express" }, { name: "MongoDB" }] },
          ],
        },
    ...
    nodes.forEach((d, i) => {
      ...
      grad.append("stop")
        .attr("offset", "100%")
        .attr("stop-color", d.depth === 0 ? "#4f46e5" : d.depth === 1 ? "#6366f1" : "#818cf8");
    });

    const simulation = d3.forceSimulation<NodeDatum>(nodes)
      .force("link", d3.forceLink<NodeDatum, d3.HierarchyLink<TreeNode>>(links).id(d => d.data.name).distance(120))
      .force("charge", d3.forceManyBody().strength(-500))
      .force("center", d3.forceCenter(width / 2, height / 2))
      .force("collide", d3.forceCollide<NodeDatum>(d => 20 + (d.children ? d.children.length * 5 : 0)));
    ...
    const label = g.append("g")
      ...
      .attr("dy", 5)
      .attr("font-size", 12)
      .attr("font-weight", d => d.depth === 0 ? "bold" : d.depth === 1 ? "bold" : "normal") // Company bold, level 1 bold, others normal
      .attr("fill", "#000");
    ...
    const amplitude = 10; // max offset in pixels

    simulation.on("tick", () => {
      t += speed;
      const offsetX = Math.sin(t) * amplitude;
      const offsetY = Math.cos(t) * amplitude;
